...
export function translateText(text: string, targetLang: Language): string {
  ...
  if (targetLang === 'hi') {
    translated = translated
      ...
      .replace(/Category \((\w+)\) matches scheme requirements \((.+)\)\./g, "श्रेणी ($1) योजना की आवश्यकताओं ($2) से मेल खाती है।")
    ...
    translated = translated
      ...
      .replace(/Category \((\w+)\) matches scheme requirements \((.+)\)\./g, "వర్గం ($1) పథకం అవసరాలకు సరిపోలుతుంది.")
